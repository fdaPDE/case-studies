...
int main (int argc, char* argv[]){
    ...
    for (int r=0; r<alphas.size(); ++r){
        ...
        Eigen::saveMarket(solution, output_dir + "fisher_kpp.mtx");
        Eigen::saveMarket(diffusion, output_dir + "diffusion.mtx");
        
        Eigen::saveMarket(test_obs, output_dir + "test_obs.mtx");
    ...
    for (int r=0; r<alphas.size(); ++r){
        ...
        for(int i=0; i<n_locs.size(); ++i){
            ...
            for(int k = 0; k < n_sim; ++k){
                ...
                obs += noise(n_locs[i], M, 0.05*std::abs(solution.maxCoeff()), gen);
