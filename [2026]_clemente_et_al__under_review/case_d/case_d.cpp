...
int main(int argc, char *argv[]){
    ...
    std::string N = std::string(argv[4]); // 5, 11, 21, 41 

    std::string mesh_dir = "input/N_" + N + "/mesh/";
    
    std::string data_dir = mesh_dir + "../" + r_param + "/";
    std::string sim_dir = data_dir + n_locs  + "/" + sim + "/";

    std::string output_dir = executable_name(argv[0]) + "/N_" +  N + "/" + r_param + "/" + n_locs + "/" + sim + "/";
    ...
    matrix_t time_mesh = read_mtx<double>("input/N_" + N + "/time_mesh.mtx");
...
//    vector_t reactions = vector_t::Ones(1);
    ...
    matrix_t test_obs = read_mtx<double>(data_dir + "test_obs.mtx").rightCols(n_times - 1);  
