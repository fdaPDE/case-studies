...
int main(int argc, char *argv[]){
    ...
    std::string n_locs = std::string(argv[1]); // "125", "250" "500" "1000" "2000" 
    std::string sim = std::string(argv[2]); // 0, ..., 29
    std::string r_param = std::string(argv[3]); // 0.20, 0.40, 0.60, 0.80 ...
    
    std::string data_dir = "input/" + r_param + "/";
    std::string sim_dir = data_dir + n_locs  + "/" + sim + "/";


    std::string output_dir = executable_name(argv[0]) + "/" + r_param + "/" +  n_locs  + "/" + sim + "/";
    ...
    matrix_t test_locs = read_mtx<double>("../simulation_1/input/" + r_param + "/test_locs.mtx");
    ...
    matrix_t exact = read_mtx<double>("../simulation_1/input/" + r_param + "/diffusion.mtx"); 
   ...
   Eigen::Matrix<double, Dynamic,2> grid = expand_grid( std::vector<vector_t>{lambdas, reactions} );
    ...
    matrix_t test_obs = read_mtx<double>("../simulation_1/input/" + r_param + "/test_obs-diffusion.mtx");  
    ...
    std::cout << "min CV error linear " << values_linear.minCoeff() << std::endl;
    std::cout << "min CV error iterative " << values.minCoeff() << std::endl;
